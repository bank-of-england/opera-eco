# Example OPERA pipeline

This example loads a simulated real-time panel from `opera.sample_realtime_data` and forecasts the quarterly target `quarterly_a` across 2026 vintages with:

- `ForecastOLS` with quarterly regressors `quarterly_b` through `quarterly_e`;
- `ForecastMIDAS` with the monthly regressor `monthly_a`.

`news_decomp` analyses nowcast news, and `forecast_combo` combines the two models. The script finishes by comparing their accuracy.

The same pipeline is available as a [Marimo notebook](../notebooks/illustration.md).
